...
def _fmt_plan_chip_dict(plan: dict) -> str:
    parts = [
        ...
        f"apply={plan.get('apply_method', False)}",
        f"response={plan.get('response_strategy', '?')}",
    ]
    present = [
        name
        for name, slot in plan.get("method_slots", {}).items()
        if isinstance(slot, dict) and (slot.get("quote") or slot.get("purpose"))
    ]
    if present:
...
def convert(path: Path) -> Path:
    ...
    meta = (
        f"method={data.get('method', cond)} | "
        f"information={data.get('information_level', '?')} | "
        f"{data.get('model_borrower','?')} | "
        f"temp={data.get('temperature','?')} | "
        f"{data.get('max_turns','?')}ターン | {date}"
    )
