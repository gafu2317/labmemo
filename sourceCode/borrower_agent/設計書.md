...
> **記事をそのまま LLM に渡す（Baseline）より、目的・希望・制約・質問項目に構造化して渡す（Structured）方が、感情表現の質と適合確認の質問の達成度が高くなるか。**
...
借り手AIは次の2つの機能を担う。
1. **感情表現**：借り手の夢・目的・想いを大家に言葉で届け、「この人に貸したい」と感じさせる
2. **適合確認の質問**：「○○に使いたいのですが、○○はありますか？」のように、借り手の目的に照らして物件が適しているかを確認する
...
一方、大家を完全自由にすると大家側のぶれがノイズになる。**物件ファクトシートに厳格に grounding した大家LLM** とし、答えられる範囲を実験設計で固定する。なお、実際の逆さま不動産では大家は物件を大雑把にしか登録しないため、`property_facts` も最小限の情報（場所・広さ・主要設備）に絞って設計する。大家LLMはLLMで質問の意図を解釈するが、出力は物件情報をシンプルに返すのみでよく、自然な会話文の生成は不要。
...
比較の主眼は「感情表現の質（Faithfulness・Persona Consistency）」と「適合確認の質問の達成度（Task Success）」の2点に置く。
