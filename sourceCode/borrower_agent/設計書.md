...
### 5.1 ケースファイル `data/eval_cases/{case_id}.yaml`
...
### 5.2 物件ファイル `data/properties/{property_id}.yaml`
...
borrower_agent/
├── src/
│   ├── models.py           TurnPlan / Case / Turn / RunResult
│   ├── llm_client.py       call_llm / call_llm_single
│   ├── planner.py          plan_turn()  ← 共通
│   ├── verifier.py         verify_utterance()  ← 共通
│   ├── borrower_policy.py  build_system_prompt()  ← 条件別
│   ├── landlord_agent.py   build_system_prompt()
│   └── dialogue_runner.py  run_dialogue()
├── prompts/
│   ├── planner.txt
│   ├── realizer_baseline.txt
│   ├── realizer_proposed.txt
│   ├── verifier.txt
│   └── landlord.txt
├── data/
│   ├── eval_cases/   実験対象ケース YAML（10件）
│   ├── cases/        全借り手記事 YAML
│   └── properties/   物件 YAML
├── scripts/
│   └── run_experiment.py
