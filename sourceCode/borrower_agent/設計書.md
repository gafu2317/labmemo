...
> **借り手の構造化プロファイルをそのまま渡す（Baseline）より、感情表現の指示を追加する（Proposed）方が、借り手の熱意・動機を大家に届ける対話品質が高くなるか。**

### 1.1 研究フレーム

対象とする大家は**貸すことには消極的だが場合によっては貸してもよいと思っている**層であり、物件の場所・広さ程度を大雑把に登録するだけでよい。その物件に対して複数の借り手エージェントがそれぞれ独立に熱意をアピールしながら対話する（**1対多の構図**）。

借り手AIは次の2つの機能を担う。研究として最も注力するのは**感情表現**である。
1. **感情表現（主）**：借り手の夢・目的・想いを短く・率直に大家に届け、「この人に貸したい」と感じさせる
2. **適合確認の質問（従）**：「○○に使いたいのですが、○○はありますか？」のように、借り手の目的に照らして物件が適しているかを確認する
...
借り手AIは自由に質問するため、大家は質問内容に応じて答える必要がある。固定台本のみでは「聞かれていないセリフを読む」形になり、適合確認の質問の検証ができない。

一方、大家を完全自由にすると大家側のぶれがノイズになる。**物件ファクトシートに厳格に grounding した大家LLM** とし、答えられる範囲を実験設計で固定する。大家LLMは質問の意図を解釈し、`property_facts` の範囲で回答する。また、「○○はないが、代わりに○○はある」という**代替案の提示**も許容し、借り手が柔軟に物件の可能性を判断できるようにする。大家のスタンスは**消極的（積極的に貸したいわけではない）**とし、借り手の感情表現が大家の態度に影響するかを観察できる設計とする。
...
| 条件 ID | 借り手の system prompt | 根拠データ | 感情表現指示 |
|---|---|---|---|
| `baseline` | ロールプレイ指示のみ | `profile`（構造化プロファイル） | **なし** |
| `proposed` | ロールプレイ指示 ＋ 感情表現ガイド | `profile`（構造化プロファイル） | **あり** |

両条件とも**同じ構造化プロファイルを入力**とし、**感情表現の指示の有無だけを変える**。これにより「感情表現指示がある方が借り手の熱意・動機を大家に届けられるか」という問いに絞った比較が可能になる。

比較の主眼は**感情表現の質（Persona Consistency・Naturalness）** に置く。適合確認（Task Success）は補助的に評価する。
...
**Baseline**: `prompts/borrower_common.txt` + `prompts/borrower_baseline.txt` + `case.profile` を整形

**Proposed**: `prompts/borrower_common.txt` + `prompts/borrower_proposed.txt` + `case.profile` を整形
...
**Baseline 固有**（`borrower_baseline.txt`）:
- 構造化プロファイルを渡し、借り手として大家に目的・確認事項を伝える
- 感情表現・継続性・共感に関する指示は一切含めない
- LLM 本来の傾向で発話させる（EvoEmo でいう vanilla 条件に対応）

**Proposed 固有**（`borrower_proposed.txt`）:
- Baseline と同じプロファイル入力に加え、以下の感情表現ガイドを追加
  - 第1発話で「なぜこの場所で活動したいか」という動機・想いを具体的に語る
  - 断られても諦めず、別の角度から熱意を伝え続ける
  - 大家の反応を受け止めつつ、感情的に共感を引き出す語り方をする
...
- `profile`（`purpose` / `wishes` / `constraints` / `questions`）を整形して埋め込む
- **感情・熱意・継続性に関する指示を一切含めない**
- 「借り手の情報をもとに大家と話してください」という最小限の指示のみ

### 7.3 借り手 Proposed `prompts/borrower_proposed.txt`

- `borrower_common.txt` の後に結合
- Baseline と同様にプロファイルを埋め込む
- **感情表現ガイドを追加：**
  - 第1発話：目的・動機・想いを具体的に語る（なぜここで活動したいか）
  - 継続性：断られても諦めず、想いを別の表現で伝え続ける
  - 共感喚起：大家の言葉を受け止め、感情的に訴えかける語り方をする
- `constraints` を **禁止事項** として強調（両条件共通）
- `questions` は「まだ確認できていないものを優先して聞いてください」と指示（両条件共通）
...
| 軸 | 定義 | 対応する機能 | v0.2 の測り方 |
|---|---|---|---|
| **Faithfulness** | 記事から直接引用または言い換えできない新情報を述べていないか | 感情表現の誠実さ | 人手（ログ読解） |
| **Persona Consistency** | 借り手の目的・価値観と合っているか | 感情表現の質 | 人手 |
| **Task Success** | `slots` の情報を大家から得られたか | 適合確認の質問の達成度 | 人手（slots_checklist を直接編集） |
| **Naturalness** | 会話として自然か | 両機能の総合的な自然さ | 人手（5段階） |

評価は研究室メンバーへのアンケートを活用し、複数名での評価を検討する。
...
| 用語 | 意味 |
|---|---|
| Baseline | 構造化プロファイルを入力するが、感情表現の指示を与えない条件 |
| Proposed | 構造化プロファイル ＋ 感情表現ガイドを与える条件（提案手法） |
| 感情表現ガイド | 「動機を語る・諦めない・共感を引き出す」という発話方針の指示 |
...
本研究（v0.3）ではプロンプト設計だけで感情表現の有効性を検証する最小単位の実験を行う。以下は将来的な拡張の候補であり、現バージョンのスコープ外である。

| 拡張方向 | 参照先 | 概要 |
|---|---|---|
| **感情ポリシーの最適化** | EvoEmo (arXiv:2509.04310) | MDPで感情状態遷移を定式化し、進化的RLで感情ポリシーを最適化する。プロンプトの固定指示より動的に最適な感情表現を実現 |
| **発話の反復改善** | DMNA (ACL 2025) | Multi-Critics + Reflexion機構で生成した発話を多面的に評価・再生成し、共感性・説得力を向上させる |
| **記憶と感情の統合** | Emotional RAG (arXiv:2410.23041) | 借り手の過去記事・記憶を感情状態に基づいて検索・参照することで、より一貫した感情的な発話を実現 |
| **ファインチューニング** | DMNA (DPO) | 借り手エージェントに対してDPOで学習させ、熱意を効果的に伝える発話スタイルをモデルに内在化させる |
| **多物件・多借り手での大規模実験** | — | 1対多の構図を完全再現し、複数物件に対して多数の借り手エージェントが並行してアピールする実験基盤に拡張 |
...
| v0.3 | 2026-06-03 | 感情表現重視への方針転換。大家に消極的スタンスと代替案提示を追加。1対多の明示 |
| v0.4 | 2026-06-09 | 比較条件の再設計。Baseline を「記事全文」から「構造化プロファイル＋感情指示なし」に変更。Proposed を「構造化プロファイル＋感情表現ガイドあり」に変更。独立変数を感情表現指示の有無に一本化。将来拡張候補を追記 |
