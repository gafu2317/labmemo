...
| 項目 | 内容 |
|---|---|
| 版 | v2.0-dev（adaptive planner。現状は v1 からのコピー、改修予定） |
| 更新 | 2026-07-15 |
...
> **固定の感情表現レシピ（Baseline = v1 の Proposed 相当）より、状況に応じて「何を・どれだけ」言うかを Planner が都度決める方式（Proposed）の方が、熱意と使い方イメージの両立した対話品質が高くなるか。**

※ 実装ステータス（2026-07-15）：ディレクトリ分離済み。Planner の動的配分ロジックは未実装で、コードは当面 v1 と同一。
...
**Planner が行為（appeal / ask 等）と発話分量を状況に応じて決め、Realizer がそれを実現する実験装置。**
（現状コードは v1 と同じ。上記は本版の目標構成。）
...
| v2.0-dev | 2026-07-15 | `v2_adaptive_planner` として分離。動的発話構成の実装はこれから |
