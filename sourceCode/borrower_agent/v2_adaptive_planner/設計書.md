...
実装ステータス（2026-07-19）：記事引用付きの熱意証拠抽出、証拠選択Planner / Realizer、大家の対話行為Controller、陶作家ケース用シナリオを実装済み。
...
1. **熱意証拠の伝達（主）**：個人的原点・本人にとっての価値・既遂行行動・継続姿勢等から、対話に合う未使用証拠を1つ選ぶ
...
[Step 1] Planner（条件別）
  ...
  → Proposed: 未使用の熱意証拠と応答方法を含む計画 JSON
  → 大家の直前の act / topic、使用済み引用、残りターンを参照

  ↓
[Step 2] Realizer（条件別）
  → Baseline: 熱意を強める特別指示なしで発話
  → Proposed: 選ばれた記事証拠を、質問への回答後に自然に伝える
...
| `no_supported_signal` | 未使用の根拠がなく、創作を避ける |
...
| `proposed` | `planner_proposed.txt` | `realizer_proposed.txt` | 検証済み熱意証拠を状況に応じて1つ選び提示 |
